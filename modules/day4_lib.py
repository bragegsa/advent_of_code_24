...
def search_matrix(data: np.array) -> int:
    ...
    # Search from left to right and vice versa
    total += xmas_search(data)
    # Search from top left to bottom right and vice versa
    total += xmas_search(get_diagonals(data))

    data_rotated = rotate_list90(data)

    # Search from top to bottom and vice versa
    total += xmas_search(data_rotated)
    # Search from top right to bottom left and vice versa
    total += xmas_search(get_diagonals(data_rotated))
    ...
    for i in range(padding, m - padding):
        ...
        for j in range(padding, n - padding):
            ...
            if (matrix[i][j] == centre_letter):
                ...
                window = [''.join(s[j - padding:j + padding + 1])
                          for s in matrix[i - padding:i + padding + 1]]
